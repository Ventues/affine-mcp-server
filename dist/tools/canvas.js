...
export function registerCanvasTools(server, gql, defaults) {
    ...
    function findSurfaceBlock(blocks) {
    ...
    function getSurfaceElementsMap(blocks) {
        ...
        const elementsOuter = surface.get('prop:elements');
        return elementsOuter.get('value');
    }
    ...
    async function addCanvasText(args) {
        const workspaceId = args.workspaceId || defaults.workspaceId;
        if (!workspaceId)
            throw new Error("workspaceId is required");
    ...
    async function listCanvasElements(args) {
        const workspaceId = args.workspaceId || defaults.workspaceId;
        if (!workspaceId)
            throw new Error("workspaceId is required");
        const { docId } = args;
        ...
        try {
            ...
            if (!surface)
                return text("No canvas elements found");
            const elementsOuter = surface.get('prop:elements');
            const elementsMap = elementsOuter.get('value');
            const elements = [];
    ...
    async function buildGraph(args) {
        ...
        try {
            ...
            const allHavePositions = nodes.every((n) => n.x !== undefined && n.y !== undefined);
            const positions = [];
            if (allHavePositions) {
