...
export function registerDocTools(server, gql, defaults) {
    ...
    server.registerTool("affine_read_doc_content", {
        title: "Read Document Content",
        description: "Read document block content via WebSocket snapshot (blocks + plain text).",
        inputSchema: {
            workspaceId: WorkspaceId.optional(),
    ...
    const readDocAsMarkdownHandler = async (parsed) => {
        const workspaceId = parsed.workspaceId || defaults.workspaceId;
        if (!workspaceId) {
            throw new Error("workspaceId is required. Provide it as a parameter or set AFFINE_WORKSPACE_ID in environment.");
        }
        ...
        try {
            ...
            const result = { docId: parsed.docId, exists: true, title, markdown };
    ...
    function blocksToMarkdownWithMap(blocks, noteBlock, title) {
        const lines = [];
        const blockLineRanges = [];
        if (title)
            lines.push(`# ${title}`, "");
        const renderBlock = (blockId, depth, listIndex) => {
            const raw = blocks.get(blockId);
            if (!(raw instanceof Y.Map))
                return;
            const flavour = raw.get("sys:flavour");
            ...
            switch (flavour) {
                ...
                case "affine:list": {
                    ...
                    for (const cid of childIds)
                        renderBlock(cid, depth + 1, listIndex);
                    break;
                }
                case "affine:code": {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i] === "" && collapsed.length > 0 && collapsed[collapsed.length - 1] === "")
                continue;
        ...
        for (let ci = 0; ci < collapseMap.length; ci++)
            reverseMap[collapseMap[ci]] = ci;
        // For lines that were collapsed away, map to the next valid collapsed line
        for (let i = lines.length - 1; i >= 0; i--) {
            if (reverseMap[i] === -1)
                reverseMap[i] = i + 1 < lines.length ? reverseMap[i + 1] : collapseMap.length;
        }
    ...
    server.registerTool("affine_delete_block", deleteBlockMeta, deleteBlockHandler);
    ...
    const moveBlockHandler = async (parsed) => {
        ...
        try {
            ...
            const flavour = block.get("sys:flavour");
            if (STRUCTURAL_FLAVOURS.has(flavour))
                throw new Error(`Cannot move structural block (${flavour}).`);
            // Remove from old parent
            const oldParentId = block.get("sys:parent");
            if (oldParentId) {
            ...
            if (pl.afterBlockId) {
                ...
                if (!ref)
                    throw new Error(`placement.afterBlockId '${pl.afterBlockId}' not found.`);
                newParentId = ref.get("sys:parent");
                ...
                if (!ref)
                    throw new Error(`placement.beforeBlockId '${pl.beforeBlockId}' not found.`);
                newParentId = ref.get("sys:parent");
                const children = ensureChildrenArray(findBlockById(blocks, newParentId));
                ...
                if (!findBlockById(blocks, newParentId))
                    throw new Error(`placement.parentId '${newParentId}' not found.`);
                const children = ensureChildrenArray(findBlockById(blocks, newParentId));
    ...
    server.registerTool("move_block", moveBlockMeta, moveBlockHandler);
    server.registerTool("affine_move_block", moveBlockMeta, moveBlockHandler);
    // ── update_doc_title ──────────────────────────────────────────────────
    ...
    server.registerTool("update_doc_title", updateDocTitleMeta, updateDocTitleHandler);
    server.registerTool("affine_update_doc_title", updateDocTitleMeta, updateDocTitleHandler);
    const updateDocMarkdownHandler = async (parsed) => {
        ...
        try {
            ...
            const newRegionMd = currentMd.slice(regionStart, idx) +
                parsed.new_markdown +
            ...
            for (const bid of affectedBlockIds) {
                ...
                if (childIdx >= 0)
                    noteChildren.delete(childIdx, 1);
    ...
    server.registerTool("affine_append_block", {
        title: "Append Block",
        description: "Append document blocks with canonical types and legacy aliases (supports placement + strict validation).",
        inputSchema: {
            workspaceId: WorkspaceId.optional(),
