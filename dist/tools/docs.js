...
export function registerDocTools(server, gql, defaults) {
    ...
    const readDocAsMarkdownHandler = async (parsed) => {
        ...
        try {
            ...
            for (const childId of noteChildIds) {
                ...
                if (!isList) {
                    if (prevWasList)
                        lines.push(""); // blank line after list group
                    listIndex.length = 0;
                }
    ...
    server.registerTool("read_doc_as_markdown", {
    ...
    async function touchDocMetaOnSocket(socket, workspaceId, docId) {
        const wsDoc = new Y.Doc();
        const snapshot = await loadDoc(socket, workspaceId, workspaceId);
        if (snapshot.missing)
            Y.applyUpdate(wsDoc, Buffer.from(snapshot.missing, "base64"));
    ...
    const updateDocMarkdownHandler = async (parsed) => {
        ...
        try {
            ...
            for (let i = 0; i < mdLines.length; i++) {
                if (charCount + mdLines[i].length >= idx) {
                    changeStartLine = i;
                    break;
                }
